...
import type { SupportedLanguage, SurveyResponse, SurveyQuestion } from '../../types';
        ...
        return (
          <span
            ...
            style={{
              ...
              lineHeight: '1',
              userSelect: 'none',
              filter: fill > 0 ? 'drop-shadow(0 0 1px rgba(212, 175, 55, 0.2))' : 'none'
            }}
          >
            <span style={{ color: 'rgba(192, 192, 192, 0.3)', position: 'absolute', top: 0, left: 0 }}>★</span>
            <span style={{
              position: 'absolute',
              top: 0,
              left: 0,
...
const ANON_SURVEY_KEY = 'standalone_anon_survey_id';

interface SatisfactionSurveyProps {
  isPublic?: boolean;
}

export function SatisfactionSurvey({ isPublic = false }: SatisfactionSurveyProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const initialHovers: Record<string, number> = {};
      activeQuestions.forEach(q => {
        initialRatings[q.id] = 0;
        initialHovers[q.id] = 0;
      });
      setRatings(initialRatings);
      setHoveredRatings(initialHovers);

      let response: SurveyResponse | null = null;
      if (user) {
        response = await surveyService.getUserSurvey(user.uid);
      } else {
        const storedAnonId = localStorage.getItem(ANON_SURVEY_KEY);
        if (storedAnonId) {
          response = await surveyService.getUserSurvey(storedAnonId);
        }
      }

      const initialTextAnswers: Record<string, string> = {};
      activeTextQuestions.forEach(q => {
        initialTextAnswers[q.id] = getTextAnswer(response, q.id);
      });
      setTextAnswers(initialTextAnswers);

      if (response) {
        setExistingSurvey(response);
  ...
  const groupOrder = useMemo(() => Array.from(new Set(questions.map(q => q.group))), [questions]);

  const handleStarClick = (key: string, value: number) => {
    setRatings(prev => ({ ...prev, [key]: value }));
  };

  const handleStarMouseEnter = (key: string, value: number) => {
    setHoveredRatings(prev => ({ ...prev, [key]: value }));
  };

  const handleStarMouseLeave = (key: string) => {
    setHoveredRatings(prev => ({ ...prev, [key]: 0 }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const key of Object.keys(ratings)) {
      const val = parseFloat(String(ratings[key]));
      sanitizedRatings[key] = isNaN(val) ? 0 : Math.max(0, Math.min(5, Math.round(val * 100) / 100));
    }

    const payload: SurveyResponse = {
      uid: user?.uid || existingSurvey?.uid || `anon_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
      name: profile ? `${profile.firstName} ${profile.lastName}` : 'Anonymous Attendee',
      email: profile?.email || 'anonymous@survey-app.com',
      country: profile?.country || 'TH',
      ratings: sanitizedRatings,
      comments: textAnswers['comments'] || '',
      suggestions: textAnswers['suggestions'] || '',
      mostInterestingSession: textAnswers['mostInterestingSession'] || '',
      improvementAreas: textAnswers['improvementAreas'] || '',
      futureTopics: textAnswers['futureTopics'] || '',
      textAnswers,
    ...
    try {
      ...
      addToast({
        ...
        message: t('custom.survey.successMessage', 'Thank you for your valuable feedback!')
      });
      setIsEditing(false);
      setShowSuccess(true);
      if (!user) {
        localStorage.setItem(ANON_SURVEY_KEY, payload.uid);
        setExistingSurvey(payload);
      } else {
        await loadData();
      }
    } catch (err: unknown) {
      addToast({
        ...
        message: (err as Error).message || 'Failed to submit evaluation'
      });
    } finally {
      setSubmitting(false);
    }
  ...
  if (!isPublic && !isAuthenticated) {
    return (
      <Card className="animate-fade-in" style={{ padding: 'var(--space-10)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-4)' }}>
        <div style={{ fontSize: '4rem' }}>🔒</div>
        <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--color-primary-800)', margin: 0 }}>
          {t('custom.survey.loginRequiredTitle', 'Authentication Required')}
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lg)', maxWidth: '500px', lineHeight: 1.6 }}>
          {t('custom.survey.loginRequiredDesc', 'Please log in to your account to complete the official conference evaluation.')}
        </p>
      </Card>
    );
  }
  ...
  if (showSuccess) {
    return (
      <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', alignItems: 'center', textAlign: 'center', padding: 'var(--space-10) 0' }}>
        <div style={{ fontSize: '5rem', marginBottom: 'var(--space-4)' }}>🎉</div>
        <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--color-primary-800)' }}>
          {t('custom.survey.successTitle', 'Submission Successful!')}
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lg)', maxWidth: '500px', lineHeight: 1.6 }}>
          {t('custom.survey.successSubtitle', 'Your feedback has been received and securely stored. We appreciate your contribution.')}
        </p>
        <button 
          className="btn btn-primary" 
          onClick={() => setShowSuccess(false)}
          style={{ marginTop: 'var(--space-4)', minWidth: '200px' }}
        >
          {t('custom.survey.viewResults', 'View My Response')}
        </button>
      </div>
    );
  }
    ...
    return (
      <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        <header className="section-header">
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--color-primary-800)', margin: 0 }}>
            📝 {t('custom.survey.title', 'Satisfaction Survey')}
          </h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', marginTop: '4px' }}>
            {t('custom.survey.submittedDesc', 'Thank you for submitting your evaluation and helping us improve.')}
          </p>
        </header>

        <Card style={{ borderLeft: '5px solid var(--color-success)', background: 'rgba(34, 197, 94, 0.05)' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#16a34a', fontWeight: 700 }}>
             <span>✅</span>
             <span>{t('custom.survey.successAlert', 'Your evaluation has been successfully logged.')}</span>
           </div>
        </Card>

        <div className="card card-gradient-border" style={{ padding: 'var(--space-8)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-4)' }}>
          <div style={{ fontSize: '3.5rem' }}>🏆</div>
          <h3 style={{ fontSize: 'var(--text-xl)', fontWeight: 800, color: 'var(--color-primary-800)', margin: 0 }}>
            {t('custom.survey.completedTitle', 'Evaluation Completed!')}
          </h3>
          <button className="btn btn-secondary btn-sm" onClick={() => setIsEditing(true)}>
            ✏️ {t('custom.survey.editButton', 'Edit My Evaluation')}
          </button>
        </div>
        ...
        <div className="card" style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
          <h4 style={{ fontSize: 'var(--text-base)', fontWeight: 800, color: 'var(--color-primary-800)', borderBottom: '1px solid var(--border-color)', paddingBottom: 'var(--space-2)', margin: 0 }}>
            📊 {t('custom.survey.yourRatings', 'Your Ratings')}
          </h4>
          ...
          <div className="survey-text-grid review-grid">
            {textQuestions.map(q => {
              ...
              return (
                ...
                >
                  <strong style={{ fontSize: 'var(--text-xs)', color: isDashed ? 'var(--color-primary-800)' : 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', marginBottom: '8px' }}>
                    {q.emoji} {q.label[lang] || q.label.en}
                  </strong>
                  <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--text-primary)', fontStyle: 'italic', lineHeight: 1.5 }}>
                    {ans ? `"${ans}"` : t('common.none', 'None')}
                  </p>
                </div>
              );
            })}
  ...
  const answeredQuestionsCount = Object.keys(ratings).filter(key => (ratings[key] || 0) > 0).length;
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      <header className="section-header">
        <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--color-primary-800)', margin: 0 }}>
          📝 {t('custom.survey.formTitle', 'Satisfaction Evaluation')}
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', marginTop: '4px' }}>
          {t('custom.survey.formSubtitle', 'Please rate your experience.')}
        </p>
        <div style={{ marginTop: 'var(--space-4)', padding: 'var(--space-3) var(--space-4)', backgroundColor: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', borderLeft: '4px solid var(--color-gold-500)', display: 'inline-block' }}>
           <strong>{t('custom.survey.scaleTitle', 'Rating Scale')}:</strong> 5 = {t('custom.survey.scale5', 'Strongly Agree')}, 4 = {t('custom.survey.scale4', 'Agree')}, 3 = {t('custom.survey.scale3', 'Neither')}, 2 = {t('custom.survey.scale2', 'Disagree')}, 1 = {t('custom.survey.scale1', 'Strongly Disagree')}
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        ...
        {/* Progress Bar */}
        <div className="survey-sticky-bar is-public">
          <div className="survey-progress-container">
            <div className="survey-progress-header">
              <span className="survey-progress-label">
                {t('custom.survey.progressLabel', 'Survey Progress')}
              </span>
              <span className="survey-progress-stats">
                {completionPercentage === 100 && <span style={{ marginRight: '6px', fontSize: '1.1em' }}>✅</span>}
                {answeredQuestionsCount} / {totalQuestionsCount} ({completionPercentage}%)
              </span>
            </div>
