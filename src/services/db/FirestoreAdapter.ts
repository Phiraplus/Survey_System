...
const DEFAULT_QUESTIONS = [
  ...
  { 
    ...
    label: { 
      ...
      ru: '3. Информация и опыт, полученные на конференции, будут полезны и могут быть practically применены в вашей работе.',
  ...
  { 
    ...
    label: { 
      ...
      ar: '9. قدم المتحدثون الضيوف عروضهم بوضوح وفتحوا المجال بنشاط للمشاركين لطرح الأسئلة أو تبادل الأفكาร.',
  ...
  { 
    ...
    label: { 
      ...
      zh: '16. ใน Lumpini 公园开展的 “Yellow Brick 体验” 提供了合适的小组活动形式，成功促进了团队建设和团结。',
  ...
  { 
    ...
    label: { 
      ...
      ar: '18. كان عشاء الوداع منسقاً بشكل جيد وممتعاً Brassica، وكان بمثابة ختام مرضٍ للغاية للبرنامج.',
...
export class FirestoreAdapter implements DatabaseAdapter, AuthAdapter {
  ...
  async getSurveyConfig(): Promise<SurveyConfig | null> {
    ...
    try {
      ...
      console.error('Firestore getSurveyConfig failed:', e);
      throw e;
    }
  }
  ...
  async submitSurvey(response: SurveyResponse): Promise<void> {
    ...
    const payload: SurveyResponse = {
      ...
      mostInterestingSession: sanitizeInput(response.mostInterestingSession || ''),
      improvementAreas: sanitizeInput(response.improvementAreas || ''),
      futureTopics: sanitizeInput(response.futureTopics || ''),
  ...
  async getUserSurvey(uid: string): Promise<SurveyResponse | null> {
    ...
    try {
      ...
      console.error('Firestore getUserSurvey failed:', e);
      return null;
    }
  }
  ...
  async getAllSurveys(): Promise<SurveyResponse[]> {
    ...
    try {
      ...
      console.error('Firestore getAllSurveys failed:', e);
      return [];
    }
  }
  ...
  async login(email: string, password: string): Promise<UserSession> {
    ...
    if (docSnap.exists()) {
      const data = docSnap.data();
      role = (data.role || 'attendee') as 'admin' | 'attendee' | 'superadmin';
      firstName = data.firstName || 'Attendee';
      lastName = data.lastName || 'Delegate';
    }
  ...
  async register(
    ...
    try {
      ...
      const newProfile = {
        ...
        country: 'TH',
        role,
        adminPasscode: passcode // Verification happens in Firebase security rules!
      };
      ...
      // Clean up user account if user document creation fails due to incorrect passcode / rules rejection
      try {
        await firebaseUser.delete();
      } catch (deleteErr) {
        console.error('Failed to clean up authentication record:', deleteErr);
      }
      throw err;
    }
  }
    ...
    return onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        try {
          ...
          console.error('AuthStateChanged load profile failed:', e);
          callback({
            uid: firebaseUser.uid,
            email: firebaseUser.email || '',
            role: 'attendee'
