...
const STORAGE_KEYS = {
  ...
  USERS: 'survey_mock_registered_users',
  SESSION: 'survey_demo_user',
};
...
const DEFAULT_QUESTIONS = [
  { 
    ...
    label: { 
      ...
      fr: '1. Les sujets abordés lors de la conférence ont répondu de manière appropriée à mis besoins et intérêts.'
    }
  },
  { 
    ...
    label: { 
      ...
      zh: '2. 会议具有时代性，并有效应对了当前的犯罪และ技术趋势（如人工智能系统、诈骗中心และ无人机）。',
  ...
  { 
    ...
    label: { 
      ...
      ar: '5. كانت جدولة وتكرาร جلسات المعرفة قصيرة المدة (Step Talk) المتكاملة طوال الأيام مناسبة ومفيدة.',
  ...
  { 
    ...
    label: { 
      ...
      ru: '14. Цอยะมอนียา ออตกฤตียา บือลา ปอเชตนอยา, เปรียสตีชนอยา อี เวสมา วเปชัตเลียยูเชย.',
  ...
  { 
    ...
    label: { 
      ...
      zh: '19. 整体社交และ娱乐项目有效地为参与者之间建立国际合作และ拓展人脉提供了机会。',
...
// In-memory callbacks to simulate firebase onAuthStateChanged
let authChangeCallbacks: ((user: UserSession | null) => void)[] = [];

export class MockAdapter implements DatabaseAdapter, AuthAdapter {
  ...
  async submitSurvey(response: SurveyResponse): Promise<void> {
    ...
    answersList = answersList.filter(s => s.uid !== response.uid);
    answersList.push({
      ...response,
  ...
  async getUserSurvey(uid: string): Promise<SurveyResponse | null> {
    ...
    const answersList: SurveyResponse[] = JSON.parse(answersStr);
    const found = answersList.find(s => s.uid === uid);
    if (!found) return null;
    return {
      ...found,
      submittedAt: new Date(found.submittedAt)
    };
  }
  ...
  async login(email: string, password: string): Promise<UserSession> {
    ...
    this.triggerAuthChange(session);
    return session;
  }
  ...
  async register(
    ...
    if (role === 'admin' && passcode !== 'SurveyAdmin2026') {
  ...
  private triggerAuthChange(user: UserSession | null) {
