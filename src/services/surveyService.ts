import { collection, doc, getDocs, setDoc, query, getDoc, deleteDoc } from 'firebase/firestore';
import { db, isFirebaseConfigured } from '../lib/firebase';
import { sanitizeInput } from '../lib/sanitizer';
import type { SurveyConfig, SurveyResponse } from '../types';

const STORAGE_KEYS = {
  ANSWERS: 'standalone_survey_answers',
  CONFIG: 'standalone_survey_config'
};
...
const DEFAULT_QUESTIONS = [
  ...
  { 
    ...
    label: { 
      ...
      fr: '4. La durée totale de la conférence et le temps alloué à chaque session étaient adéquat et approprié.'
    }
  },
  { 
...
export const surveyService = {
  /**
   * Fetch survey configuration (questions)
   */
  getSurveyConfig: async (): Promise<SurveyConfig | null> => {
    ...
    try {
      const docSnap = await getDoc(doc(db, 'config', 'survey_questions'));
      if (docSnap.exists()) {
      ...
      console.error('Failed to load configuration from Firestore:', e);
      return null;
    }
  },

  /**
   * Update survey configuration (questions)
   */
  updateSurveyConfig: async (config: SurveyConfig): Promise<void> => {
    ...
    const docRef = doc(db, 'config', 'survey_questions');
    await setDoc(docRef, {
      ...config,
      updatedAt: new Date()
    });
  ...
  getUserSurvey: async (uid: string): Promise<SurveyResponse | null> => {
    if (!isFirebaseConfigured || !db) {
      ...
      if (answers) {
        const parsedList: SurveyResponse[] = JSON.parse(answers);
        const userSurvey = parsedList.find(s => s.uid === uid);
        if (userSurvey) {
          return {
            ...userSurvey,
            submittedAt: new Date(userSurvey.submittedAt)
          };
        }
      }
      return null;
    ...
    try {
      const docSnap = await getDoc(doc(db, 'surveys', uid));
      if (docSnap.exists()) {
        const data = docSnap.data();
        return {
          ...data,
      ...
      console.error('Failed to load user survey:', e);
      return null;
    }
  },

  /**
   * Save a user's satisfaction survey response
   */
  submitSurvey: async (survey: SurveyResponse): Promise<void> => {
    const sanitizedTextAnswers: Record<string, string> = {};
    if (survey.textAnswers) {
      for (const key of Object.keys(survey.textAnswers)) {
        sanitizedTextAnswers[key] = sanitizeInput(survey.textAnswers[key] || '');
      }
    }

    const payload: SurveyResponse = {
      ...survey,
      comments: sanitizeInput(survey.comments || ''),
      suggestions: sanitizeInput(survey.suggestions || ''),
      mostInterestingSession: sanitizeInput(survey.mostInterestingSession || ''),
      improvementAreas: sanitizeInput(survey.improvementAreas || ''),
      futureTopics: sanitizeInput(survey.futureTopics || ''),
      textAnswers: sanitizedTextAnswers,
      submittedAt: survey.submittedAt ? new Date(survey.submittedAt) : new Date()
    };

    if (!isFirebaseConfigured || !db) {
      const answersStr = localStorage.getItem(STORAGE_KEYS.ANSWERS);
      let answersList: SurveyResponse[] = answersStr ? JSON.parse(answersStr) : [];
      
      // Remove old submission if exists
      answersList = answersList.filter(s => s.uid !== payload.uid);
      answersList.push(payload);
      
      localStorage.setItem(STORAGE_KEYS.ANSWERS, JSON.stringify(answersList));
      return;
    }

    const docRef = doc(db, 'surveys', payload.uid);
    await setDoc(docRef, {
      ...payload,
  ...
  deleteSurvey: async (uid: string): Promise<void> => {
    if (!isFirebaseConfigured || !db) {
      const answersStr = localStorage.getItem(STORAGE_KEYS.ANSWERS);
      if (answersStr) {
        let answersList: SurveyResponse[] = JSON.parse(answersStr);
        answersList = answersList.filter(s => s.uid !== uid);
        localStorage.setItem(STORAGE_KEYS.ANSWERS, JSON.stringify(answersList));
      }
      return;
    }

    const docRef = doc(db, 'surveys', uid);
    await deleteDoc(docRef);
  },

  /**
   * Fetch all survey responses for admin report dashboard
   */
  getAllSurveys: async (): Promise<SurveyResponse[]> => {
    if (!isFirebaseConfigured || !db) {
      const answersStr = localStorage.getItem(STORAGE_KEYS.ANSWERS);
      if (answersStr) {
        const parsedList: SurveyResponse[] = JSON.parse(answersStr);
    ...
    try {
      ...
      console.error('Failed to load surveys from Firestore:', e);
      return [];
    }
  }
};
